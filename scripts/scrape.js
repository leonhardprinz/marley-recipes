...
async function scrape() {
  ...
  const maxRecipes = recipeUrls.length; // scrape all
  const urlsToScrape = recipeUrls.slice(0, maxRecipes);
  ...
  for (const url of urlsToScrape) {
    try {
      ...
      // Additional fallback: meta og:image
      if (!image) {
        try {
      ...
      // Additional fallback: data-src or srcset from first img
      if (!image) {
        try {
          const firstImg = await p.locator('img').first();
      ...
      const add = (v) => {
        ...
        else if (typeof v === 'string') v.split(',').forEach((x) => tagsArr.push(x.trim()));
      };
      if (recipeNode) {
      ...
      recipes.push({
        ...
        url,
        image,
        tags: Array.from(new Set(tagsArr.filter(Boolean).map((t) => t.toLowerCase()))),
        totalTimeMinutes,
        calories,
        ingredients,
      });
